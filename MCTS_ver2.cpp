...
int main() {
   ...
   // double score = std::numeric_limits<double>::max();
    ...
    TreeNode *root = new TreeNode(head, *scheduled, *alltasks);
        ...
        while (!flag) {
            if (current->children.size() == 0){
               ...
               flag = true;
                ...
                if (current->children.size() < current->readyTasks.size()*totalCPUNum) {
                    for (int x = 0; x < current->readyTasks.size(); x++) {
                        for (int y = 1; y <= totalCPUNum; y++) {
                            for (int z = 0; z < current->children.size(); z++) {
                                if ( current->readyTasks[x].procID != current->children[z]->scheduledTasks[current->children[z]->scheduledTasks.size()-1].procID || current->readyTasks[x].cpu != current->children[z]->scheduledTasks[current->children[z]->scheduledTasks.size()-1].cpu) {
                                    vector<Task> scheduledTasks;
                                    vector<Task> readyTasks;
                                    readyTasks = current->readyTasks;
                                    scheduledTasks = current->scheduledTasks;
